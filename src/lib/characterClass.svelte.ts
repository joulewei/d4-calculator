...
export class Character {
    ...
    public additionalMultipliers: { [key: string]: ConditionalMultiplier } = $state({
        heirOfPerdition: { active: true, value: 80, displayName: "Heir of Perdition (Mythic)", isCrit: false },
    ...
    private _calculate(
        ...
        const prodAdditionalNormal = Object.values(additional)
            .filter(m => m.active).filter(m => !m.isCrit)
		...
		console.log({prodAdditionalNormal, prodAdditionalCrit})

        const normalDamage = weaponDmg * (1 + sumAdditivesNormal) * skillMult * prodFactorsNormal * prodAdditionalNormal * this.vulnerableBaseDamage;
        ...
        const critDamage = weaponDmg * (1 + sumAdditivesCrit) * skillMult * prodFactorsCrit * prodAdditionalNormal * prodAdditionalCrit * this.critBaseDamage;
    ...
    public getCritChanceGain(step: number = 5): number {
        return this._calculate(this.additives, this.multiplier, this.additionalMultipliers, this.weaponDmg, this.critChance + step) - this.currentDamage;
    }

    public getSkillDamageGain(step: number = 10): number {
        return this._calculate(this.additives, this.multiplier, this.additionalMultipliers, this.weaponDmg, this.critChance, this.skillDamage + step) - this.currentDamage;
    }

    public getSkillCoeffGain(step: number = 5): number {
        return this._calculate(this.additives, this.multiplier, this.additionalMultipliers, this.weaponDmg, this.critChance, this.skillDamage, this.skillCoeff + step) - this.currentDamage;
    }
